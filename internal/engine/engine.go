...
// CountEmbedded returns the count of LIVE engrams (excluding soft-deleted and
// archived) that have had embeddings generated, i.e. the DigestEmbed flag is
// set. Returns -1 on error.
//
// vault == "" counts across the whole store; a non-empty vault scopes the count
// to that vault's own engrams (#802). The 0x11 DigestFlags keyspace is
// deliberately global (see docs/internals/keyspace-registry.md), so the
// vault-scoped path is a different, more expensive query: scan the vault's
// engram IDs and look up each one's flags, rather than scanning DigestFlags.
//
// Neither path uses CountWithFlag, which scans 0x11 directly. That keyspace is
// never cleaned up on ClearVault or a hard delete (see PebbleStore.ClearVault's
// doc comment), so it can count flags orphaned by engrams that no longer exist.
// That let EmbeddedCount exceed the live total in production, which made the
// embed-status API's `indexing` bool (embedded < total) compute false even with
// a real, actively-processing backlog. CountEngramsWithFlag and
// CountEmbeddedInVault both scan the live engram keyspace instead, so their
// results can never exceed a live total computed the same way.
...
// soft-deleted and archived) — the same live-state filter CountEmbedded
// applies. Returns -1 on error. vault == "" counts across the whole store; a
// non-empty vault scopes it, so the two sides of a per-vault coverage ratio
// agree on which rows are "in the vault".
//
// The embed-status API must compare EmbeddedCount against THIS, not Stat's
// EngramCount: that counter (e.engramCount) only decrements on a hard delete,
// never on soft delete or archive, so it still counts engrams CountEmbedded
// deliberately excludes. Comparing the two would let EmbeddedCount sit
// permanently below EngramCount for a vault holding embedded-then-soft-deleted
// engrams, with the `indexing` bool reporting true forever even though there is
// nothing left to embed.
func (e *Engine) CountEmbeddableTotal(ctx context.Context, vault string) int64 {
	if vault != "" {
		count, err := e.store.CountLiveEngramsInVault(ctx, e.store.ResolveVaultPrefix(vault))
		if err != nil {
			return -1
		}
		return count
