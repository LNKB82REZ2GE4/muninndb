...
// TestRegisterMigrations_IncludesV8 keeps the runner and the migration in sync —
// v3 shipped referenced only from tests and never ran in production.
//
// FORK NUMBERING: upstream registers the #726 replication relocation as v5. This
// fork already shipped its own v5/v6 to live data dirs, so upstream's v5 and v6
// are renumbered to 8 and 9 here — see RegisterMigrations.
func TestRegisterMigrations_IncludesV8(t *testing.T) {
	r := &Runner{}
	...
	if !found {
		t.Fatalf("migration v8 (upstream v5, #726 replication prefix relocation) is not registered")
	}
	// The exact value is pinned by TestRegisterMigrations_IncludesV9 (the
	// current head); here only the floor matters.
	if got := MaxRegisteredVersion(); got < 8 {
		t.Errorf("MaxRegisteredVersion() = %d; want >= 8 — the refuse-newer downgrade guard keys off this", got)
	}
}

// TestRunner_RefusesDowngradeAfterV8 is the downgrade story for #726 (upstream
// v5, registered as v8 in this fork): a store
...
func TestRunner_RefusesDowngradeAfterV8(t *testing.T) {
	db := v5TestDB(t)
	seedLegacyStore(t, db, 8)
