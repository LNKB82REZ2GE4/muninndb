...
// FORK NUMBERING: upstream registers the #683 entity vault-scoping migration as
// v6. This fork already shipped its own v5/v6 to live data dirs, so upstream's
// v5/v6 are renumbered to 8/9 here — see RegisterMigrations.
func TestRegisterMigrations_IncludesV9(t *testing.T) {
	r := &Runner{}
	...
	if !found {
		t.Fatalf("migration v9 (upstream v6, #683 entity vault scoping) is not registered")
	}
	if got := MaxRegisteredVersion(); got != 9 {
		t.Errorf("MaxRegisteredVersion() = %d; want 9 — the refuse-newer downgrade guard keys off this", got)
	}
}

// TestRunner_RefusesDowngradeAfterV9 is the downgrade story for #683 (upstream
// v6, registered as v9 in this fork): a store
...
func TestRunner_RefusesDowngradeAfterV9(t *testing.T) {
	db := v6TestDB(t)
