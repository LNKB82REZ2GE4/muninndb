...
func (b *pebbleStoreBatch) WriteAssociation(ctx context.Context, ws [8]byte, src, dst ULID, assoc *Association) error {
	...
	// This fork's durable per-vault marker (0x31) is staged in the SAME batch as
	// the edge, alongside upstream's in-memory contradictsQueued generation bump
	// above. The two are complementary, not rivals: the generation invalidates
	// upstream's scan cache, while the marker is the O(1) gate that keeps a
	// multi-million-association vault from paying a capped scan per query.
