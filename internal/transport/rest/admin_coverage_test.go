...
// Asserts the handler forwards the requested vault to BOTH sides of the ratio —
// sharing scope matters, because a vault-scoped numerator over an instance-wide
// denominator is a coverage ratio that looks plausible and is wrong.
//
// FORK: the denominator is CountEmbeddableTotal, not Stat's EngramCount.
// EngramCount only decrements on a hard delete, never on soft delete or archive,
// while the numerator excludes both — comparing them lets EmbeddedCount sit
// permanently below TotalCount and latches `indexing` true with nothing left to
// embed. Both sides now apply the identical live-state filter AND the same #802
// vault scoping, which is what this test pins.
func TestHandleEmbedStatus_HonorsVaultParam(t *testing.T) {
	store := newTestAuthStore(t)
	eng := &MockEngine{
		countEmbeddableTotalByVault: map[string]int64{"alpha": 5, "beta": 500},
	...
	if eng.lastCountTotalVault == nil || *eng.lastCountTotalVault != "alpha" {
		got := "<nil>"
		if eng.lastCountTotalVault != nil {
			got = *eng.lastCountTotalVault
		}
		t.Errorf("CountEmbeddableTotal was not called with vault=alpha, got %q", got)
	}
