...
| **0x31** | ws | 1B state | **per-vault declared-contradiction marker (COG-29's O(1) recall gate).** `0x01` = a declared `contradicts` association has been written to this vault; `0x00` = a COMPLETE scan of 0x03 found none; **ABSENT = UNKNOWN and must never be read as clean.** Written in the SAME Pebble batch as the contradicts edge itself (`storage.MarkDeclaredContradictionInBatch`, called from every 0x03 writer: `WriteAssociation`, engram/batch stores, archive restore, adjacency), so a crash cannot leave a durable contradiction behind a `clean` marker. **Monotone** — `SetDeclaredContradictionMark` refuses to demote `0x01` to `0x00`, because clearing it would require proving no declared edge remains anywhere in the vault (the vault-wide scan this marker exists to avoid) and being wrong turns contradiction honesty off silently; the accepted residual is a resolved vault continuing to run a read-only, bounded phase. Backfilled for pre-existing edges by **migration v6**, and RELOCATED here from 0x2F by **migration v7** (see below) (uncapped single pass over 0x03; 0.77s for 3.8M associations on a production vault). Recall also persists a verdict it proves itself: a COMPLETE clean probe writes `0x00`, a capped probe writes NOTHING. Cleared by `ClearVault` (a reused vault name must not inherit a stranger's marker). NOT cloned or exported: bulk copies (`MergeVaultData`, `CloneVaultData`, `ImportVaultData`) move raw 0x03/0x04 bytes without calling `WriteAssociation`, so each reconciles the TARGET explicitly via `ReconcileDeclaredContradictionMarkAfterBulkCopy` — source `yes` promotes the target, otherwise the target's marker is DELETED back to UNKNOWN. Never leave a `none` marker standing over imported edges: a merge into a proven-clean target was reproducibly able to hide a declared contradiction forever. **Concurrency:** the `none` write is a check-then-set and takes the vault's stripe of `declaredContradictionLocks` (`stripedRWMutex`) EXCLUSIVELY across its Get+Set; every edge writer holds the same stripe SHARED across its batch commit (`holdDeclaredContradictionMark`). Without it a clean probe demoted a just-declared contradiction in ~30% of contended rounds. `internal/index/adjacency` stages the marker without the lock and is safe only because nothing constructs it. |
...
`0x32`–`0x3F` and `0x47`+ are free for new storage/auth keys (0x2B–0x31 are now
...
0x2F the replication keyspace (#726), 0x30 upsert forward-index (#556), 0x31
declared-contradiction marker (this fork);
0x40–0x45 are allocated: 0x40/0x41 capability, 0x42–0x45 auth; 0x46 glob-scope API
key index, this fork).

**`0x2F` and `0x30` are RESERVED FOR UPSTREAM and must not be reused by this fork.**
Upstream #726 relocates the replication keyspace onto `0x2F` (partitioned by a second
discriminator byte: `0x2F|0x01|seq_be64` log entries, `0x2F|0x02|name` metadata) and
#556 takes `0x30` for the upsert forward index. This fork originally allocated `0x2F`
to the declared-contradiction marker, whose key shape `0x2F|ws(8)` would have landed
inside upstream's log-entry or metadata sub-range for any vault whose hash begins
`0x01` or `0x02`. Migration **v7** (`PurgeLegacyDeclaredContradictionMark`) erases
every legacy `0x2F` marker key and rebuilds the marker at `0x31`. It is registered
BEFORE the renumbered upstream replication relocation so it cannot delete a
replication record; that ordering is load-bearing and pinned by a test.

### Fork vs upstream migration numbering

This fork applied its own migrations **v5** (glob-scope API key index, `0x46`) and
**v6** (declared-contradiction marker backfill) to live data directories before the
upstream 0.11.0 merge, so those numbers are permanently this fork's. Upstream's own
v5 (replication keyspace relocation) and v6 (vault-scoped entity records) are
therefore renumbered to **v8** and **v9** here. The migration runner stores a single
high-water mark, so renumbering the already-applied fork migrations instead would
have caused upstream's v5/v6 to be silently skipped on every existing data dir. (`0x29`/`0x40`/`0x41`
also appear in `internal/transport/mbp/frame.go` as **wire opcodes**, a different
keyspace; coincidental, safe, but confusing. Prefer `0x32+` for new storage prefixes.)
