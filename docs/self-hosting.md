...
└── muninn.logdest      Where the daemon's logs actually go, recorded at every
                        startup — a concrete path, or "stderr:inherited" when
                        no --log-file/MUNINN_LOG_FILE is set. `muninn logs`
                        reads this instead of assuming <dataDir>/muninn.log,
                        so it never presents another daemon lifetime's stale
                        log as current when a service manager captures the
                        real output elsewhere.
~/.muninn/
└── mcp.token           MCP bearer token (auto-generated by muninn init)
```

**Under a process supervisor** (launchd, systemd, Docker), point `--log-file`/
`MUNINN_LOG_FILE` at the same file your supervisor already captures, so
