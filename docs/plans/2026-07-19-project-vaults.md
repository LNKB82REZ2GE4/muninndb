...
> **Revision note:** after a full read of `docs/` (three parallel reader agents, 2026-07-19), the design was corrected in place: score-normalised merging replaced with weighted RRF (§4.2), the global entity registry acknowledged (§4.1), provisioning moved off the admin surface onto a new scoped `POST /api/vaults` (§5.1), and key-space/durability/migration constraints folded into §3.3, backfill fidelity into §5.2, promoter twin-collision/provenance rules into §5.4, archival into §7.
>
> **Operational facts to remember at implementation time** (from the same audit, no design change needed): fresh project vaults read *low* on the coherence score (orphan-heavy until auto-association catches up) — never wire automation off coherence thresholds for young vaults, and expect backfill to spike orphan ratio; dual writes double semantic-trigger push volume for any subscriber watching both tiers, and double-increment global entity `MentionCount` (documented-intended, but it stops being a count of distinct facts); the novelty LRU is in-memory only, so a backfill split across restarts relies wholly on `op:`/`idempotent_id` for dedup (which it has); associations cap at 256 per engram — dual-writing plus auto-assoc (10/write) burns this faster in small dense vaults; consider per-tier retrieval modes (project=`recent`, general=`semantic`) as a complement to RRF weights; the Playwright E2E suite has zero coverage of key/vault-management UI, so fork auth changes ship without regression cover there.
...
- **Learning-signal caveats:** Hebbian/PAS state is per-vault, so a naive N-vault loop records N separate PAS transition sets for one logical recall — `ActivateMulti` must decide this explicitly (default: record transitions only in the top-weighted vault). Hebbian co-activation events must be fed **raw per-vault scores**, never normalised ones — RRF ranks are for merging only; normalised scores (which push a small vault's results toward 1.0) would systematically inflate association strengths there (co-activation signal is `sqrt(scoreA × scoreB)`).
- **Brief:** `brief_mode` summaries are computed per single-vault ActivateResponse; a merged call needs the brief recomputed over the merged set (or takes the project vault's brief and appends the general tier's top lines).
...
- **Twin collision:** a distilled engram will often exceed the 0.70-Jaccard novelty threshold against the *deliberate-remember copy already in agent-memory* (the "remember goes to both" path), silently creating REFINES links and inflating that vault's duplication-pressure counter. Before writing, the promoter checks for an existing twin (by `op:` hash, then near-duplicate match) and prefers **evolving/consolidating the existing copy** over inserting a new one.
- **Provenance encoding:** custom entity-relationship types outside the built-in 11 are silently stored as `0xFF` — never mint a `promoted_from` relation; provenance stays tags + shared entities + engram-level assoc types (`supersedes`, `references`).
